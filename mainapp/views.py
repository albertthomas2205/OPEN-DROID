...
def view_all_admin_users(request, role):
    try:
        ...
        users = HealthcareUser.objects.filter(role=role,is_deleted=False)
...
@api_view(['DELETE'])
@permission_classes([IsAuthenticated])
def soft_deactivate_admin_user(request, user_id):
    try:
        if request.user.role != 'admin':
            return Response({
                'status': 'error',
...
def soft_delete_admin_user(request, user_id):
    try:
        ...
        # ----- FETCH USER (ONLY NOT DELETED) -----
        user_instance = HealthcareUser.objects.filter(
            id=user_id,
            is_deleted=False
        ).first()
        ...
        user_instance.is_active = False   # Recommended
        user_instance.save()
...
@api_view(['DELETE'])
@permission_classes([IsAuthenticated])
def soft_deactive_patient(request, patient_id):
    try:
        if request.user.role not in ['admin', 'nurse']:
            return Response({
                'status': 'error',
...
def soft_delete_patient(request, patient_id):
    try:
        ...
        # Get Patient
        patient = Patient.objects.filter(pk=patient_id, is_deleted=False).first()
        if not patient:
            return Response({
                'status': 'error',
                'message': 'Patient not found.',
        ...
        patient.slot_assigned = None
        patient.updated_by = request.user
        patient.save()

        # Remove scheduled slots if any
        ScheduledSlots.objects.filter(patient=patient).delete()
        ...
        logger.exception(f"Exception in soft_delete_patient: {e}")
        return Response({
            'status': 'error',
            'message': 'Internal server error.',
            'data': None
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_deleted_patients(request):
    try:
        # Role Check
        if request.user.role not in ['admin', 'nurse']:
            return Response({
        ...
        deleted_patients = Patient.objects.filter(is_deleted=True)

        serializer = PatientSerializer(deleted_patients, many=True)
        ...
        logger.exception(f"Exception in get_deleted_patients: {e}")
        return Response({
            'status': 'error',
            'message': 'Internal server error.',
            'data': None
...
def assign_designation(request):
    try:
        ...
        if serializer.is_valid():
            serializer.save()
            SlotDataModel.objects.filter(id=slot_assigned).update(is_booked=True)
            return Response({
                'status': 'success',
