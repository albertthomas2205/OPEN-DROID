...
def view_all_slot(request):
    try:
        ...
        slots = SlotDataModel.objects.filter(is_deleted=False).order_by('-created_at')
        serializer = SlotDataModelSerializer(slots, many=True)
        slots_count = SlotDataModel.objects.filter(is_deleted=False).count()
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def view_not_booked_slot(request):
    try:
        if request.user.role not in ['admin', 'nurse']:
            return Response(
        ...
        slots = SlotDataModel.objects.filter(
            is_deleted=False,
            is_booked=False
        ).order_by('-created_at')
        ...
        logger.exception(f"Exception in view_all_slot: {e}")
        return Response(
...
@api_view(['DELETE'])
@permission_classes([IsAuthenticated])
def soft_deactive_slot(request, slot_id):
    try:
        # ----- ROLE CHECK -----
        if request.user.role not in ['admin', 'nurse']:
            return Response({
...
def soft_delete_slot(request, slot_id):
    try:
        ...
        # ----- FETCH SLOT (only non-deleted) -----
        slot_instance = SlotDataModel.objects.filter(
            id=slot_id,
            is_deleted=False
        ).first()
        ...
        # ----- HANDLE BED RELEASE -----
        if slot_instance.bed_name:
            BedDataModel.objects.filter(
                id=slot_instance.bed_name.id
            ).update(is_booked=False)

        # ----- SOFT DELETE -----
        slot_instance.is_deleted = True
        slot_instance.is_active = False  # Optional but recommended
        slot_instance.updated_by = request.user
        slot_instance.save()
        ...
        logger.exception(f"Exception in soft_delete_slot: {e}")
        return Response({
            'status': 'error',
            'message': 'Internal server error.',
            'data': None
